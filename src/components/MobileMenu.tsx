...
export function MobileMenu() {
  ...
  return <>
      ...
      {/* Mobile Menu Overlay */}
      {isOpen && <div className="lg:hidden fixed inset-0 z-30 pt-14">
          {/* Backdrop */}
          ...
          <nav className="relative bg-[hsl(var(--sidebar-bg))] text-white w-64 h-full overflow-auto animate-slide-in">
            <ul className="p-4 space-y-2">
              {filteredNavItems.map(item => <li key={item.path}>
                  <NavLink to={item.path} end={item.path === '/'} onClick={handleNavClick} className={({
              isActive
            }) => `flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${isActive ? 'bg-[hsl(var(--sidebar-active))] font-medium' : 'hover:bg-[hsl(var(--sidebar-hover))]'}`}>
                    <item.icon size={20} />
            ...
            {user && <div className="p-4 border-t border-[hsl(var(--sidebar-hover))]">
                ...
                <button onClick={handleLogout} className="w-full flex items-center gap-2 px-4 py-2 rounded-lg text-sm hover:bg-[hsl(var(--sidebar-hover))] transition-colors">
                  <LogOut size={16} />
                  Sign Out
                </button>
              </div>}
