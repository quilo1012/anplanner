...
export function useWorkOrderChecklist(workOrderId: string | undefined, problemName: string | undefined) {
  ...
  const toggleItem = async (checklistId: string, completed: boolean, userId: string | undefined): Promise<OperationResult> => {
    ...
    try {
      const completedBy = completed ? (userId ?? null) : null;
      const completedAt = completed ? new Date().toISOString() : null;
      const { error } = await supabase
        .from('checklist_responses' as never)
        .upsert(
          { work_order_id: workOrderId, checklist_id: checklistId, completed, completed_by: completedBy, completed_at: completedAt } as never,
          { onConflict: 'work_order_id,checklist_id' } as never,
        );
